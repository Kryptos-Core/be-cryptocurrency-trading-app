...
- Kiểm tra `NODE_ENV` không phải là `production`
- Kiểm tra server đã start chưa
- Kiểm tra route `/api/docs` có đúng không

### Authentication không work

- Đảm bảo đã click "Authorize" và nhập token
- Token phải là valid JWT token
- Kiểm tra token chưa expired

### DTOs không hiển thị

- Đảm bảo đã thêm `@ApiProperty()` hoặc `@ApiPropertyOptional()`
- Kiểm tra DTO đã được import đúng trong controller

## Advanced Configuration

Cấu hình nâng cao có thể được chỉnh sửa trong `src/config/swagger.config.ts`:

- Thêm servers
- Thêm tags
- Customize UI
- Thêm security schemes khác
