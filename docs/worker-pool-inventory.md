...
| Outbox | Relay flush + dispatch integration handlers | Bull queue `outbox-relay` + Redis lock `outbox:relay:lock` | Scale consumers; keep idempotent handlers; xem [ARCHITECTURE.md](ARCHITECTURE.md) |
