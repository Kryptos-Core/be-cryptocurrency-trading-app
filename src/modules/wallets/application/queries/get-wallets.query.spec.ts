...
function makeRow(overrides: Partial<{ wallet_id: string; currency_id: string; available: string; frozen: string; currency_symbol: string; currency_name: string }> = {}) {
 return {
 wallet_id: 'wid-1',
 currency_id: 'cid-1',
 available: '100',
...
describe('GetWalletsQuery', () => {
 ...
 walletRepo.findByUser.mockResolvedValue([makeRow(), makeRow({ wallet_id: 'wid-2', currency_id: 'cid-2', available: '200', frozen: '0', currency_symbol: 'ETH', currency_name: 'Ethereum' })]);
 ...
 walletRepo.findByUser.mockResolvedValue([makeRow({ available: null as any, frozen: null as any })]);
