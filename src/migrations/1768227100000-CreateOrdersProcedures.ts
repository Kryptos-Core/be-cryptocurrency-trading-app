...
export class CreateOrdersProcedures1768227100000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE PROCEDURE sp_order_find_by_id(IN p_order_id BIGINT)
      READS SQL DATA
      BEGIN
    ...
    // ----- sp_order_create (multi-statement: use runProcedureSql) -----
...
proc_label: BEGIN
  ...
  INSERT INTO orders (user_id, pair_id, side, type, price, amount, filled_amount, status, time_in_force, reserved_quote, reserved_base, client_order_id, idempotency_key, created_at, updated_at)
  VALUES (p_user_id, p_pair_id, p_side, p_type, p_price, p_amount, 0, 'OPEN', COALESCE(p_time_in_force, 'GTC'), v_reserve_quote, v_reserve_base, p_client_order_id, p_idempotency_key, NOW(6), NOW(6));
...
proc_label: BEGIN
  ...
  DECLARE v_pair_id INT DEFAULT NULL;
  DECLARE v_quote_currency_id INT DEFAULT NULL;
  DECLARE v_base_currency_id INT DEFAULT NULL;
...
`;
    ...
    // ----- sp_order_count_by_user (IF/ELSE -> runProcedureSql) -----
